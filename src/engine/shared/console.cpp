					...
					else
					{
						...
						if(Result.HasVictim() && Result.GetVictim() == CResult::VICTIM_ALL)
						{
							for(int i = 0; i < MAX_CLIENTS; i++)
							{
								...
								if(!pCommand->m_LuaCallback.empty())
								{
									luabridge::LuaRef Func = luabridge::getGlobal(pCommand->m_pLuaState, pCommand->m_LuaCallback.c_str());
									if(Func.isValid() && Func.isFunction())
									{
										 luabridge::LuaResult Res = Func(Result);
										 if(Res.hasFailed())
...
void CConsole::LuaRegister(lua_State *L)
{
	if(lua_gettop(L) < 6)
	{
		luaL_error(L, "function expected 6 arguments");
		return;
	}

	const char *pName = luaL_checkstring(L, 2);
	const char *pParams = luaL_checkstring(L, 3);
	int Flags = luaL_checkinteger(L, 4);
	const char *pCallbackName = luaL_checkstring(L, 5);
	const char *pHelp = luaL_checkstring(L, 6);
	...
	pCommand->m_pfnCallback = 0x0;
	pCommand->m_pUserData = 0x0;
	pCommand->m_pLuaState = L;

	pCommand->m_pName = pName;
	pCommand->m_pHelp = pHelp;
	pCommand->m_pParams = pParams;
	pCommand->m_LuaCallback = pCallbackName;
