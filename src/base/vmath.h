...
public:
	...
	vector2_base lua_operator_sub(const vector2_base &vec) const { return vector2_base(x - vec.x, y - vec.y); }
	vector2_base lua_operator_add(const vector2_base &vec) const { return vector2_base(x + vec.x, y + vec.y); }
	vector2_base lua_operator_mul(const vector2_base &vec) const { return vector2_base(x * vec.x, y * vec.y); }
	vector2_base lua_operator_div(const vector2_base &vec) const { return vector2_base(x / vec.x, y / vec.y); }

	std::string tostring() const
	{
		char aBuf[64];
		str_format(aBuf, sizeof(aBuf), "vec2(%.4f, %.4f)", x, y);
