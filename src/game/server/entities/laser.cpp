...
void CLaser::DoBounce()
{
	...
	if(Res)
	{
		if(!HitCharacter(m_Pos, To))
		{
			...
			if(!GameServer()->m_pController->m_TeleOuts[z - 1].empty())
			{
	...
	else if(m_Owner >= 0)
	{
		...
		bool IsSwitchTeleGun = GameServer()->Collision()->GetSwitchType(MapIndex);
		bool IsBlueSwitchTeleGun = GameServer()->Collision()->GetSwitchType(MapIndex);
