...
void CCharacterCore::Tick(bool UseInput, bool DoDeferredTick)
{
		...
		if(!this->m_HookHitDisabled && m_pWorld && m_Tuning.m_PlayerHooking)
		{
			...
			for(int i = 0; i < MAX_CLIENTS; i++)
			{
				...
				if(closest_point_on_line(m_HookPos, NewPos, pCharCore->m_Pos, ClosestPoint))
				{
					...
					{
						if(m_HookedPlayer == -1 || !m_pHookedDummy || distance(m_HookPos, pCharCore->m_Pos) < Distance)
						{
			...
			for(CCharacterCore *pCore : m_pWorld->m_vDummies)
			{
				...
				if(closest_point_on_line(m_HookPos, NewPos, pCore->m_Pos, ClosestPoint))
				{
					...
					{
						if(m_HookedPlayer == -1 || !m_pHookedDummy || distance(m_HookPos, pCore->m_Pos) < Distance)
						{
							m_TriggeredEvents |= COREEVENT_HOOK_ATTACH_PLAYER;
							m_HookState = HOOK_GRABBED;
	...
	if(m_HookState == HOOK_GRABBED)
	{
		...
		m_HookTick++;
		if(m_pHookedDummy || m_HookedPlayer != -1)
		{
			bool NoChr = m_HookedPlayer != -1 && (m_pWorld && !m_pWorld->m_apCharacters[m_HookedPlayer]);
			bool HookOut = (m_HookTick > SERVER_TICK_SPEED + SERVER_TICK_SPEED / 5);
